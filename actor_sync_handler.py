...
class UnifiedSyncHandler:
    ...
    def sync_emby_person_map_to_db(self, update_status_callback: Optional[callable] = None, stop_event: Optional[threading.Event] = None):
            ...
            for person_batch in emby_handler.get_all_persons_from_emby(self.emby_url, self.emby_api_key, self.emby_user_id, stop_event):
                ...
                for person_emby in person_batch:
                    if stop_event and stop_event.is_set():
                        ...
                        if update_status_callback: update_status_callback(-2, "同步已取消")
                        return
                    ...
                    person_data_for_db = {
                        "emby_id": emby_pid,
                        "name": person_name,
                        "tmdb_id": {k.lower(): v for k, v in (person_emby.get("ProviderIds") or {}).items()}.get("tmdb"),
                        "imdb_id": {k.lower(): v for k, v in (person_emby.get("ProviderIds") or {}).items()}.get("imdb"),
                        "douban_id": {k.lower(): v for k, v in (person_emby.get("ProviderIds") or {}).items()}.get("douban"),
                    }
        ...
        logger.info(f"✅ 冲突/跳过插入: {stats['conflicts']} 条")
