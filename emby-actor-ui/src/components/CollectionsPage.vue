<!-- src/components/CollectionsPage.vue (风格统一版) -->
<template>
  <n-layout content-style="padding: 24px;">
          ...
          <n-gi v-for="item in missingCollections" :key="item.emby_collection_id">
            ...
            <n-card class="glass-section" :bordered="false" content-style="display: flex; padding: 0; gap: 16px;">
              ...
              <div class="card-poster-container">
                <n-image
                  lazy
                  :src="getCollectionPosterUrl(item.poster_path)"
                  class="card-poster"
                  object-fit="cover"
                >
                  <template #placeholder>
                    <div class="poster-placeholder"><n-icon :component="AlbumsIcon" size="32" /></div>
                  </template>
                </n-image>
              </div>
              ...
              <div class="card-content-container">
                ...
                <div class="card-actions">
                  ...
                  <n-tooltip>
                    <template #trigger>
                      <n-button text @click="openInEmby(item.emby_collection_id)" target="_blank">
                  ...
                  <n-tooltip>
                    <template #trigger>
                      <n-button text tag="a" :href="`https://www.themoviedb.org/collection/${item.tmdb_collection_id}`" target="_blank" :disabled="!item.tmdb_collection_id">
    ...
    <n-modal
      ...
      :title="selectedCollection ? `缺失影片 - ${selectedCollection.name}` : ''"
      :bordered="false"
      size="huge"
    >
...
import { 
  ...
  NPopconfirm, NTooltip, NGrid, NGi, NCard, NImage, NEllipsis, NSpin, NAlert, NModal
} from 'naive-ui';
import { 
  SyncOutline, AlbumsOutline as AlbumsIcon, EyeOutline as EyeIcon
...
const subscribeToMovie = async (tmdb_id, title) => {
  ...
  try {
    ...
    message.success(`《${title}》已成功提交到 MoviePilot 订阅！`);
