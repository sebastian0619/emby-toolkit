...
const translateAllFields = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/actions/translate_cast_api', { cast: editableCast.value });
...
const fetchMediaDetails = async () => {
  ...
  try {
    const response = await axios.get(`/api/media_api_edit_details/${itemId.value}`);
...
const handleSaveChanges = async () => {
  ...
  try {
    ...
    await axios.post(`/api/update_media_cast_api/${itemDetails.value.item_id}`, payload);
